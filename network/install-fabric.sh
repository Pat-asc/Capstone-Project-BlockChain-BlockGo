...
download() {
    ...
    # Using wget -c to make the download more robust and resumable
    wget -c "${URL}" -O "${BINARY_FILE}"
    if [ $? -ne 0 ]; then
        echo "==> There was an error downloading the binary file."
        return 22
    fi

    # Unpack and then clean up
    tar xz -f "${BINARY_FILE}" -C "${DEST_DIR}" || rc=$?
    rm "${BINARY_FILE}"
    if [ -n "$rc" ]; then
        echo "==> There was an error unpacking the binary file."
        return 22
    fi
    echo "==> Done."
}
